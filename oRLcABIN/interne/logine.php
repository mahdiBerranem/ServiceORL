<?php 
  if (!empty($_POST)) {
     ...
     if (empty($_POST['password'])){
      $erreur = "votre mot de passe n'est pas valide" ;
     } 
 $mdp = $_POST['password'] ;
      if (empty($erreur)) { 
        $bdd = new PDO('mysql:host=localhost;dbname=serviseorl', 'root', '');
        $bdd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); 
        $req = $bdd->prepare("SELECT * FROM user WHERE mdp = '".$mdp."' ");
        $req-> execute(['mdp' => $_POST['password'] ]);
        $us = $req->fetch(PDO::FETCH_OBJ);
        if ($us) {
          $mdp2=$us->mdp;
       if ($mdp == $mdp2) {
         header('location:listecours.php');
      }else{
      $erreur = "votre mot de passe est incorecte" ;
      }
      }
  }else{
...
<div class="ab">   
 ...
 <li><a href="/oRLcABIN/interne/logine.php">Platforme interne</a> </li>
</br>
</br>
...
<h1>Connexion</h1>
      <form method="POST" action="">
        <p>mot de passe</p>  
        <input type="password" name="password" placeholder="Entrer le mot de passe" style="color:black;">
...
.enghauche li {text-align:justify;}
.enghauche strong{font-size: 14px; color:#3A8DE9 }
...
.toggle{
  position:fixed;
   ...
   right:10%;
   left:75%;
   background:url(../btnespacepro.png) no-repeat;
   cursor: pointer; 
}
