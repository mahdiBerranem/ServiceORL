...
if(isset($_POST['forminscription'])) {
   ...
   if(!empty($_POST['nom'])AND !empty($_POST['prenom']) AND !empty($_POST['numero'])AND !empty($_POST['medecin']) AND !empty($_POST['mail']) AND !empty($_POST['mail2']) AND !empty($_POST['mdp']) AND !empty($_POST['mdp2'])) {
      $pseudolength = strlen($nom);
      if($pseudolength <= 255) {
         if($mail == $mail2) {
            if(filter_var($mail, FILTER_VALIDATE_EMAIL)) {
          $reqmail = $bdd->prepare("SELECT * FROM infirmiere WHERE `email`= ?");
              $reqmail->execute(array($mail));
               $mailexist = $reqmail->rowCount();
               if($mailexist == 0) {
                  if($mdp == $mdp2) {
                     $insertmbr = $bdd->prepare("INSERT INTO infirmiere (`nom`, `prenom`, `numero`, `doc_id`, `email`, `motpass`) VALUES(?, ?, ?, ?, ?, ?)");
                     $insertmbr->execute(array($nom, $prenom, $numero, $medecin, $mail, $mdp));                 
...
<html>
<head>
   <title>Insciption ifirmiere</title>
...
<div class="row justify-content-center">
                    <div class="col-md-8">
                        <div class="card">
                            <div class="card-header">Insciption ifirmiere </div>
                            <div class="card-body">
                                ...
                                <form class="form-horizontal" method="post" action="">
                                    ...
                                    <div class="form-group">
                                        <label for="name" class="cols-sm-2 control-label">Choisir Le id medecin (travaille)</label>
                                        <div class="cols-sm-10">
                                            <div class="input-group">
                                                <span class="input-group-addon"><i class="fa fa-user fa" aria-hidden="true"></i></span>
...
<?php
$reponse = $bdd->query('SELECT * FROM docteur ORDER BY doc_id');
while ($choix = $reponse->fetch())
{
?>
<?php echo '<option value="'.$choix['doc_id'].'" selected>'.$choix['doc_id'].'</option>';
  echo '<option value="'.$choix['nom'].'" disabled selected>'.$choix['nom'].'</option>'; 
