...
//preparation requete 
 ...
 if (!empty($_POST['search'])) {
  ...
  and (adressepar=?)";
   ...
   $sql="SELECT * FROM `dossier malade` WHERE adressepar=? ORDER BY numdossier DESC " ;
 }
...
<div class="enghauche">
<h2>Liste Dossier Médicale</h2> 
<br>
<form method="post" action="dosmed.php"> 
<input type="submit" name="" value="Créer" class="btn btn-info" title="Nouveau dossier medical"></form>
<br><br>
...
<td> 
<a href="affichdosmed.php?numdossier=<?= $dossiermedical->numdossier ;?>&&nompat=<?= $dossiermedical->nompat; ?>" class=" btn btn-info"> afficher</a>
<a href="modifierdos.php?numdossier=<?= $dossiermedical->numdossier ;?>&&nompat=<?= $dossiermedical->nompat; ?>" class="btn btn-info"> modifier</a>
<a onclick="return confirm('vous-êtes sûr de vouloir supprimer cette entrée ?')" href="supdosmed.php?numdossier=<?= $dossiermedical->numdossier ;?>" class="btn btn-danger"> supprimer</a>
</td> 
<?php endforeach ; ?> 
</tr>
</table>
